...
func (g *Graph) MaximizeEdgesOrderWeightedDynamic() ([]int, [][][4]int, []int) {
	...
	// Initialize heap with nodes, setting initial contribution as the node's degree
	for node := range g.AdjacencyList {
		initContribution := len(g.AdjacencyList[node]) + len(g.DanglingEdgeList[node])
		// initialContribution := len(neighbors) // Use degree as initial contribution
