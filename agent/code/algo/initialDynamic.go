...
func (g *Graph) MaximizeEdgesOrderInitDynamic(nodeId int) ([]int, [][][4]int, []int) {
	nodeNum := len(g.AdjacencyList)
	_, ok := g.AdjacencyList[nodeId]
	if !ok {
		fmt.Printf("Invalid initial nodeId %v. Only %v nodes.\n", nodeId, nodeNum)
		return nil, nil, nil
	}
