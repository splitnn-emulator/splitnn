...
int container_run(
    ...
    if ((base_dir == NULL) || (hostname == NULL) || (img_rootfs_dir == NULL)) {
        fprintf(stdout, "Empty base_dir or hostname\n");
        return -1;
    }
...
void do_run_command(int argc, char *argv[]) {
    ...
    // fprintf(stdout, "Operation: run\n");
    // fprintf(stdout, "Netns Path: %s\n", netns_path);
    // fprintf(stdout, "Base Directory: %s\n", base_dir);
    // fprintf(stdout, "Hostname: %s\n", hostname);
    // fprintf(stdout, "Image Rootfs Directory: %s\n", img_rootfs_dir);
...
void do_exec_command(int argc, char *argv[]) {
    ...
    char *endptr;
    pid_t pid = strtol(argv[2], &endptr, 10);
    if (*endptr != '\0') {
    ...
    char **cmdline = &argv[3];
    int cmdline_len = argc - 3;
    if (cmdline_len == 0) {
        fprintf(stderr, "Error: 'cmdline' cannot be empty.\n");
        print_usage(argv[0]);
    }

    fprintf(stdout, "Operation: exec\n");
...
void do_kill_command(int argc, char *argv[]) {
    int pid = 0;
    const char *log_file_path = NULL;
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(op, "run") == 0) {
        do_run_command(argc, argv);
    } else if (strcmp(op, "exec") == 0) {
        do_exec_command(argc, argv);
    } else if (strcmp(op, "kill") == 0) {
        do_kill_command(argc, argv);
    } else {
